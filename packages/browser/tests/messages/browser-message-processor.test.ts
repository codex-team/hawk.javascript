...
import { BrowserAddonMessageProcessor } from '../../src/messages/browser-addon-message-processor';
...
describe('BrowserMessageProcessor', () => {
  ...
  it('should set window dimensions, userAgent, and url', () => {
    const result = new BrowserAddonMessageProcessor().apply(makePayload());
  ...
  it('should merge with existing payload addons', () => {
    const payload = makePayload({ addons: { consoleOutput: [{ message: 'log' }] } });
    const result = new BrowserAddonMessageProcessor().apply(payload);
